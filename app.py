...
def index():
    if request.method == 'POST':
        try:
            ...
            txid = str(uuid.uuid4())
            new_transaction = Transaction(txid=txid, net_amount=round(net_amount, 2), charge_amount=round(gross_amount, 2), payout_pix_key=payout_pix_key, payout_pix_key_type=payout_pix_key_type)
            ...
            if pix_code and qr_code_base64:
                ...
                flash('Não foi possível gerar a cobrança Pix. Verifique suas chaves de API no painel do Render e os logs.', 'error')
                return redirect(url_for('index'))
...
def calculate():
    ...
    try:
        ...
        commission = gross_amount * PERCENTAGE_COMMISSION
        net_amount = gross_amount - commission - FIXED_FEE
        return jsonify({
